...
namespace dssp {
	...
	namespace {
		...
		bool initializeONNXDriver(LangCore::Manager *langMgr, DSSP_Device device) {
			...
			// TODO: this indicates that the ONNX driver should reuse synthrt's ONNX runtime. Not a elegant way to do so
			onnxArgs->loadFromProcess = true;
...
bool DSSP_InitializeLanguageConversion(DSSP_Device device) {
	LangCore::Logger::setLogCallback(dssp::logLangCore);

	const auto langMgr = LangCore::Manager::instance();

	const auto defaultPluginDir = dssp::getPluginRootDirectory();
	...
	const std::filesystem::path packagesRootDir = dssp::getPackagesRootDirectory();
	langMgr->addPackagePath(packagesRootDir);

	dssp::initializeONNXDriver(langMgr, device);

	std::string errorMessage;
	langMgr->initialize(errorMessage);
	if (!langMgr->initialized()) {
		dssp::g_languageConversionErrorMessage = std::move(errorMessage);
		return false;
	} else {
		dssp::g_languageConversionErrorMessage.clear();
		return true;
	}
}

const char *DSSP_GetLanguageConversionErrorMessage(void) {
	return dssp::g_languageConversionErrorMessage.c_str();
}

DSSP_Pronunciations DSSP_ConvertLanguage(DSSP_Lyrics lyrics) {
	const auto *input = dssp::getLyrics(lyrics);
	auto pronunciations = std::make_unique<dssp::Pronunciations>();
	pronunciations->reserve(input->size());
	std::vector<LangCore::G2pInput> g2pInput;
	g2pInput.reserve(input->size());
	std::ranges::transform(*input, std::back_inserter(g2pInput), [](const dssp::Lyric &lyric) {
	...
	std::ranges::transform(g2pResults, std::back_inserter(*pronunciations), [](const LangCore::G2pRes &res) {
		dssp::Pronunciation pronunciation;
